...
let _exact_sch =
  ...
  let open Tuple in
  custom [String; String; Array String]
    (fun {name;file;namespace} -> [Modname.to_string name;file;namespace])
    (fun [name;file;namespace] -> {name= Modname.v name;file;namespace})
